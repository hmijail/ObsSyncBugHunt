...
#
# CONTAINER_ENGINE (podman/docker) is normally exported by the Makefile, which resolved it once;
# the fallback keeps the script runnable by hand.
set -eu
...
until "$CONTAINER_ENGINE" exec "$node" pgrep -f /opt/obsidian/obsidian >/dev/null 2>&1; do
...
while [ "$(date +%s)" -lt "$deadline" ]; do
  n=$((n + 1))
  report=$("$CONTAINER_ENGINE" exec "$node" timeout 45 "$hc" 2>/dev/null | tail -1)
  shot_bytes=0; windows=0; notes=ERR
...
while [ "$(date +%s)" -lt "$deadline" ]; do
  n=$((n + 1))
  out=$("$CONTAINER_ENGINE" exec "$node" timeout 20 "$cli" sync:status 2>/dev/null || true)
