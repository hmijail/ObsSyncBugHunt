...
# THE CONTAINER RUNTIME'S INTERNALS — how fast an engine rebuilds a veth, reprograms NAT, and lets
# traffic flow again — and engines change. That is exactly the kind of thing that can quietly
...
#
# Usage: scripts/check-net.sh [rounds] [outage_seconds] [budget_seconds]
#        make check-net                       (rounds=3, outage=10s, budget=1.0s)
#        make check-net ROUNDS=10 BUDGET=0.5
#
...
# Deliberately the pinned address of a node that does NOT exist in a normal run (n9 -> .109), so
# this can never collide with a live n1/n2 on the same network.
IP=10.89.0.109

echo "check-net: engine=$CONTAINER_ENGINE network=$NET rounds=$ROUNDS outage=${OUTAGE}s budget=${BUDGET}s"

# The test network must already exist with the right subnet — `make net` owns that logic, and
# `make check-net` depends on it. Fail loudly rather than quietly creating a differently-shaped one.
if ! $CONTAINER_ENGINE network inspect "$NET" >/dev/null 2>&1; then
  echo "check-net: network '$NET' does not exist — run 'make net' first" >&2
  exit 1
fi
...
$CONTAINER_ENGINE run -d --name "$NAME" --network "$NET" --ip "$IP" "$IMG" \
  bash -c 'while :; do
             t=$(cut -d" " -f1 /proc/uptime)
             timeout 1 bash -c "echo > /dev/tcp/8.8.8.8/53" >/dev/null 2>&1
             echo "$t $?" >> /probe.log
             sleep 0.05
           done' >/dev/null || { echo "check-net: could not start the probe container" >&2; exit 1; }

uptime_now() { $CONTAINER_ENGINE exec "$NAME" cut -d' ' -f1 /proc/uptime; }

# Wait for the probe loop to actually be succeeding before trusting any measurement.
i=0
while [ "$i" -lt 60 ]; do
  ok=$($CONTAINER_ENGINE exec "$NAME" sh -c 'grep -c " 0$" /probe.log 2>/dev/null || echo 0')
  [ "${ok:-0}" -gt 2 ] && break
  i=$((i + 1)); sleep 1
done
if [ "${ok:-0}" -le 2 ]; then
  echo "check-net: the probe container never reached 8.8.8.8:53 even while connected —" >&2
  echo "  this machine has no outbound connectivity, so the check can't say anything." >&2
  exit 1
fi
echo "check-net: probe container up and reaching the internet"

fails=0
r=1
while [ "$r" -le "$ROUNDS" ]; do
  $CONTAINER_ENGINE network disconnect "$NET" "$NAME" >/dev/null 2>&1
  sleep "$OUTAGE"

  u_pre=$(uptime_now)
  $CONTAINER_ENGINE network connect --ip "$IP" "$NET" "$NAME" >/dev/null 2>&1 \
  ...
  ip_now=$($CONTAINER_ENGINE exec "$NAME" sh -c 'hostname -I 2>/dev/null | tr -d " \n"')
  first_ok=$($CONTAINER_ENGINE exec "$NAME" awk -v u="$u_pre" '$1 > u && $2 == 0 { print $1; exit }' /probe.log)
...
echo
if [ "$fails" -eq 0 ]; then
  echo "check-net: PASS — a $OUTAGE""s outage reconnects within ${BUDGET}s on its pinned IP."
  echo "  D/C is a link blip: established connections stall and resume, they don't reset."
  exit 0
fi
echo "check-net: FAIL ($fails/$ROUNDS rounds) — reconnection is slower than ${BUDGET}s or lost the pinned IP." >&2
...
echo "  see docs/DESIGN.md ('network identity: pinning the same IP across reconnects')." >&2
exit 1
