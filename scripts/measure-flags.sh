...
#
# CONTAINER_ENGINE (podman/docker) and IMAGE come from the environment — `make` exports the
# engine, and node images are tagged by Obsidian version, so name the one to measure:
#   IMAGE=obsidian-node:1.13.7 scripts/measure-flags.sh "baseline" "" 10
# (`make list-images` lists what's built.)
set -u
...
case "$img" in
  ...
  *) echo "measure-flags: IMAGE='$img' has no version tag — node images are tagged by Obsidian" >&2
     echo "  version now. Pass e.g. IMAGE=obsidian-node:1.13.7 ('make list-images' lists them)." >&2
     exit 1 ;;
esac
...
running=$("$CONTAINER_ENGINE" ps --filter "network=$net" --format '{{.Names}}' 2>/dev/null | tr '\n' ' ')
...
for i in $(seq 1 "$N"); do
  "$CONTAINER_ENGINE" rm -f "$name" >/dev/null 2>&1 || true
  "$CONTAINER_ENGINE" run -d --name "$name" --network "$net" \
    -e OBSIDIAN_FLAGS="$flags" -e OBSIDIAN_RENDER_BUDGET=8 \
    -v "$secrets":/secrets:ro "$img" >/dev/null

  att=""
  for _ in $(seq 1 90); do
    att=$("$CONTAINER_ENGINE" exec "$name" cat /var/log/obsidian-attempts 2>/dev/null || true)
    if [ -n "$att" ]; then break; fi
    if [ "$("$CONTAINER_ENGINE" inspect -f '{{.State.Running}}' "$name" 2>/dev/null || echo false)" != true ]; then break; fi
    sleep 1
  done
