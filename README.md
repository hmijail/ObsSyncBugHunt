...
| `LOCAL_VAULT_PIN` | `--local-vault-pin` | off | opt-in: every local-node content command (read/append/create/open/files/etc.) explicitly targets the vault captured at startup via `vault=<name>`, instead of whatever's focused in the GUI. Confirmed live: this only reaches a vault that's already open as its OWN separate Obsidian window — it does not open/switch to one by name. So the captured vault must stay open as a window (not necessarily focused) for the whole soak; any OTHER window is then free to use without disrupting these calls. Fails fast at startup if the vault name couldn't be captured |

**Keep the local Obsidian GUI visible during a soak.** Observed repeatedly: while the window is hidden (macOS Cmd+H), its `sync:status` turns slow or gets stuck reporting `syncing` for a long, unpredictable stretch (minutes to hours) — the baseline-sync gate at the start of every rep waits unboundedly for it, so the whole soak stalls until the window is brought back to the front. If it must stay out of the way, `defaults write md.obsidian NSAppSleepDisabled -bool YES` (relaunch Obsidian after) is worth trying as a secondary option, though bringing the window forward is the more reliable fix. For an unattended overnight run, also consider wrapping the soak in `caffeinate -dis` to stop the Mac itself from sleeping.
